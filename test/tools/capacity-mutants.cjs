...
const MUTANTS = [
  ...
  ['u13 the usage push gated on the strip collectionRevision', INDEX,
    ...
    'onChange: () => { const was = lastPushedCapacityStrip; pushCapacityStrip(); if (lastPushedCapacityStrip !== was) pushAgentUsage(); pushAgentImpact(); inboxWake?.onCapacityChange(); }'],
  ...
  ["c15 a capacity publication does not push impacts", INDEX,
    "onChange: () => { pushCapacityStrip(); pushAgentUsage(); pushAgentImpact(); inboxWake?.onCapacityChange(); }",
    "onChange: () => { pushCapacityStrip(); pushAgentUsage(); inboxWake?.onCapacityChange(); }"],
